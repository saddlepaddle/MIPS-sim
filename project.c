...
/* Gets the instruction from the PC only if the instruction is grabbed from a
 * valid location in MEM 
 */
...
/* Based on the opcode given, sends the signals needed to the various other
...
int ALU_operations(unsigned data1,unsigned data2,unsigned extended_value,
	...
	// If the ALUOp is invalid, halt
	if (ALUOp < 0 || ALUOp > 7) return 1;
	
	// If the ALUOp signals an R-type instruction, determine which the instruction
	// and assign to ALUOp using funct
	if (ALUOp == 7) {
		switch (funct){
			case 0x20: // add
				ALUOp = 0x0;
...
/* If the signal is given to read or write memory, the memory is fetched or
 * written to as told after checking if the location is valid 
 */
...
/* Writes to the register if the register address is not 0 (i.e. given) and if
 * the signal to write is given 
 */
